...
describe UsersController do
  ...
  describe "as admin user" do
    ...
    it "should allow creating users" do
      lambda { post :create, params: { user: FactoryGirl.attributes_for(:user) } }.should change(User, :count).by(1)
    ...
    it "should allow updating users" do
      ...
      lambda { put :update, params: { id: @user.id, user: user } }.should change { @user.reload.name }.to(user[:name])
      response.should redirect_to(users_url)
    end

    it "should prevent creating users when the user limit has been reached" do
      @account.expects(:reached_user_limit?).returns(true)
      lambda { post :create, params: { user: FactoryGirl.attributes_for(:user) } }.should_not change(User, :count)

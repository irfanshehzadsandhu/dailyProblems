...
describe Subscription do
  ...
  it "should need payment info when no card is saved and the plan is not free" do
    Subscription.new(:plan => @basic).needs_payment_info?.should be_truthy
  end

  it "should not need payment info when the card is saved and the plan is not free" do
    Subscription.new(:plan => @basic, :card_number => 'foo').needs_payment_info?.should be_falsey
  end

  it "should not need payment info when no card is saved but the plan is free" do
    @basic.amount = 0
    Subscription.new(:plan => @basic).needs_payment_info?.should be_falsey
  end

  it "should find expiring trial subscriptions" do
    Subscription.expects(:includes).with(:subscriber).returns(
      mock('finder').tap {|m| m.expects(:where).with({ :state => 'trial', :next_renewal_at => (7.days.from_now.beginning_of_day .. 7.days.from_now.end_of_day) }) }
    )
  ...
  it "should find active subscriptions needing payment" do
    Subscription.expects(:includes).with(:subscriber).returns(
      mock('finder').tap {|m| m.expects(:where).with({ :state => 'active', :next_renewal_at => (Time.now.beginning_of_day .. Time.now.end_of_day) }) }
    )
  ...
  it "should find active subscriptions needing payment in the past" do
    Subscription.expects(:includes).with(:subscriber).returns(
      mock('finder').tap {|m| m.expects(:where).with({ :state => 'active', :next_renewal_at => (2.days.ago.beginning_of_day .. 2.days.ago.end_of_day) }) }
    )
    ...
    describe "with a credit card" do
      ...
      describe "storing the card" do
        ...
        describe "without a trial period" do
          ...
          it "should set the renewal date to now" do
            @basic.trial_period = nil
            @basic.renewal_period = 3
            @sub = Subscription.new(:plan => @basic)
            @sub.expects(:next_renewal_at=).with(@time)
            @sub.save.should be_truthy
          end
        end
      end
    end
  ...
  describe "" do
    ...
    describe "when failing to store the card" do
      it "should return false and set the error message to the processor response" do
        ...
        @sub.store_card(@card).should be_falsey
        @sub.errors.full_messages.should include('Forced failure')
      end
    end
    ...
    describe "when storing the credit card" do
      describe "successfully" do
        ...
        after(:each) do
          ...
          @sub.expects(:save)
          @sub.store_card(@card).should be_truthy
        end
        ...
        describe "sends receipt" do
          ...
          it "unless there was no charge" do
            @sub.next_renewal_at = @time.advance(:days => 2)
            lambda { @sub.store_card(@card).should be_truthy }.should change(SubscriptionPayment, :count).by(0)
